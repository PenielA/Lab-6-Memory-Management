...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
  node_t *available_block_node = NULL; 
	node_t *curr = freelist->head;
	node_t *prev = NULL; 
	while(curr){
		if (curr->blk->end - curr->blk->start >= blocksize){
			available_block_node = curr; 
			break;
		} 
		prev = curr;
		curr = curr->next;
	}
	if (!available_block_node){
		...
		if (!prev){
			...
			prev->next = available_block_node->next;
		}
	}
	int blk_end = available_block_node->blk->end; 
	available_block_node->blk->pid = pid;
	available_block_node->blk->end = available_block_node->blk->start + blocksize - 1;
	list_add_ascending_by_address(alloclist, available_block_node->blk);
	if (available_block_node->blk->end != blk_end){
		block_t *fragment = (block_t*)malloc(sizeof(block_t));
		fragment->pid = 0;
		fragment->start = available_block_node->blk;
